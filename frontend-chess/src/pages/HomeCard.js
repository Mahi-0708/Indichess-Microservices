...
function HomeCard({ onLoginSuccess }) {
    ...
    const checkAuth = async () => {
      try {
        ...
        if (response.ok) {
          ...
          onLoginSuccess(); // Fetch user details if already logged in
  ...
  return (
    <div className="home-card">
      {isAuthenticated ? (
        ...
        <div style={{ color: 'white', fontSize: '24px' }}>Redirecting to Home...</div>
      ) : (
        <div>
          {showSignup ? (
            <SignupCard handleToggleSignup={handleToggleSignup} />
          ) : (
            <LoginCard handleToggleSignup={handleToggleSignup} onLoginSuccess={onLoginSuccess} />
          )}
        </div>
      )}
    </div>
